...
export async function saveResourceToDB(
  data: ResourceInput,
...
export async function getUserResourcesFromDB(): Promise<
...
export async function getResourceFromDB(
  _id: string,
): Promise<ActionResult<SerializedResource>> {
  try {
    await db();
...
export async function setPublishedStatusInDB(
  _id: string,
  published: boolean,
): Promise<ActionResult<SerializedResource>> {
  try {
...
export async function getLatestResourcesFromDB(
  page = 1,
...
export async function getResourceBySlugFromDB(
  slug: string,
): Promise<ActionResult<SerializedResource>> {
  try {
    await db();
